...
import static com.dreamsportslabs.guardian.constant.Constants.SCOPE;
...
public class OidcTokenService {
  ...
  private Single<OidcTokenResponseDto> authorizationCodeFlow(
    ...
    return authenticateClient(requestDto, tenantId, authorizationHeader)
        ...
        .flatMap(
            codeSessionModel ->
                codeSessionService
                    .deleteCodeSession(requestDto.getCode(), tenantId)
                    .andThen(Single.just(codeSessionModel)))
        .flatMap(
            codeSessionModel ->
                scopeService
                    .getOidcScopes(tenantId)
                    .flatMap(
                        oidcScopes ->
                            userService.getOidcUser(
                                getUserFilters(codeSessionModel, oidcScopes), headers, tenantId))
                    .flatMap(
                        userResponse -> {
                          GenerateOidcTokenDto generateOidcTokenDto =
                              getGenerateOidcTokenDto(codeSessionModel, tenantId, userResponse);
  ...
  private Map<String, String> getUserFilters(
      CodeSessionModel codeSessionModel, List<String> oidcScopes) {
    oidcScopes.retainAll(codeSessionModel.getConsentedScopes());
    Map<String, String> filters = new HashMap<>();
    filters.put(SCOPE, String.join(" ", oidcScopes));
