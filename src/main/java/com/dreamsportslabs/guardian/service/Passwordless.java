...
public class Passwordless {
  ...
  private Single<OtpGenerateModel> getOtpGenerateModel(String cacheKey) {
    return passwordlessDao
        ...
        .map(
            model -> {
              if (System.currentTimeMillis() / 1000 > model.getExpiry()) {
                passwordlessDao.deleteOtpGenerateModel(cacheKey);
                throw INVALID_STATE.getException();
              }
              return model;
            });
  ...
  public Single<OtpGenerateModel> initOtpOnly(
    ...
    if (state != null) {
      otpGenerateModel =
          this.getOtpGenerateModel(passwordlessDao.getCacheKeyForOtp(tenantId, state));
    } else {
      state = generateState();
      TenantConfig tenantConfig = registry.get(tenantId, TenantConfig.class);
      updateContactTemplate(tenantConfig, requestDto.getContact());
      otpGenerateModel = this.createOtpGenerateModel(requestDto, headers, tenantId, state);
    }

    String cacheKey = passwordlessDao.getCacheKeyForOtp(tenantId, state);
  ...
  private Single<OtpGenerateModel> createOtpGenerateModel(
      ...
      String state) {
    TenantConfig tenantConfig = registry.get(tenantId, TenantConfig.class);
    updateContactTemplate(tenantConfig, dto.getContact());
