...
public class AdminConfig {
  ...
  public CompletionStage<Response> createAdminConfig(
    ...
    return adminConfigService
        .createAdminConfig(tenantId, requestDto, userIdentifier)
  ...
  public CompletionStage<Response> updateAdminConfig(
    ...
    requestDto.validate();
    return adminConfigService
        .updateAdminConfig(tenantId, requestDto, userIdentifier)
        .map(config -> AdminConfigResponseDto.from(tenantId, config))
        .map(response -> Response.ok(response).build())
        .toCompletionStage();
  }
  ...
  @DELETE
  public CompletionStage<Response> deleteAdminConfig(
      @HeaderParam("tenant-id") String tenantId,
      @HeaderParam("user-identifier") @NotBlank(message = USER_IDENTIFIER_HEADER_REQUIRED)
          String userIdentifier) {
    return adminConfigService
        .deleteAdminConfig(tenantId, userIdentifier)
